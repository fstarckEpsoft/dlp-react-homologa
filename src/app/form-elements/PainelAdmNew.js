...
const App = () => {
    ...
    const [newAmbiente, setNewAmbiente] = useState(["AWSSERVERDEV"]);
            ...
            {admin ? (
                <>
                    <div className="background">
                        ...
                        <div className='dividerAdm' />

                        <div className='botoes'>
                            <button type="button" className='botao-funcao' title={"Adiciona novo cliente ao S3 e BD."} onClick={() => getData() > setIsModalNewClient(true)}>Criar novo Cliente</button>
                            <button type="button" className='botao-funcao' title={"Edita as informações de um cliente existente."} onClick={() => getData() > setIsModalNewClient(true)}>Configurar Grupo</button>
                            <button type="button" className='botao-funcao' title={"Adiciona novo cliente ao S3 e BD."} onClick={() => getData() > setIsModalNewClient(true)}>Checa Parâmetros</button>
                        ...
                        <div>
                            <a href="http://localhost:3003/flashsafe/conf/form-Elements" class="botao-sair">Sair</a>
                        </div>
                                ...
                                <div className="modal-find-opcoes-tabela">
                                    ...
                                    <div className="box-inputs">
                                        <div className='input-vars'>
                                            <p>Configuração VARS2</p>
                                            {data.map((item, index) => (
                                                item.Nome !== 'newDiscovery' && (
                                                    <div key={index} className='input-vars22'>
                                                        <label htmlFor={`parametro-${index}`} for="parametro" className='label-vars'>
                                                            {item.Nome}
                                                        </label>
                                                        {item.Nome === 'mapperAtivado' ? (
                                                            <select id={`parametro-${index}`} value={item.Valor} onChange={(e) => handleSelectNewChange(e, index)}>
